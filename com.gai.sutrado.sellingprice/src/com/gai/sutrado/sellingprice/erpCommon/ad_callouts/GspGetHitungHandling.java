...
public class GspGetHitungHandling extends SimpleCallout {
  ...
  protected void execute(CalloutInfo info) throws ServletException {
    ...
    String strHandling = info.getStringParameter("inphandlingP", null);
    ...
    hasilBagi = desimalHandling.divide(new BigDecimal("100"));
    hasilHandling  = desimalNetsales.multiply(hasilBagi);
    hasilHandling = hasilHandling.setScale(2, BigDecimal.ROUND_HALF_UP);
    hasilTambah = desimalSelling.add(hasilHandling);
    hasilUnitPrice = hasilTambah.divide(desimalQty);
	hasilUnitPrice = hasilUnitPrice.setScale(2, BigDecimal.ROUND_HALF_UP);
	
	hasilSellingPrice = desimalSales.add(desimalFreight).add(desimalInsurance).add(hasilHandling);
    // inject the result into the response
    info.addResult("inphandlingV",hasilHandling.toString());
